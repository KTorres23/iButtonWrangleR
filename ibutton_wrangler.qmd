---
title: "iButtonWrangleR: Automate iButton Data Processing"
format: html
---

iButton data should be stored in folders by their plots. The following convention is used: plot_PLOTNUMBER_DEPLOYDATE_PICKUPDATE The following is an example of a folder name following the convention: plot_1a_0630-0702

**NOTE**: This script was designed for humidity and temperature data. If other data types are present, exercise caution in that they are processed correctly by the functions in this script.
...
install.packages(c("pacman", "tidyverse", "readxl", "here"))
#"MuMIn", "performance", "easystats")
...
```{r function_ibutton_extract_raw}
...
```{r function_ibutton_read_datetimes}
...
```{r example_ibutton_read_datetimes}
...
```{r function_ibutton_view_metadata}
...
```{r example_ibutton_view_metadata}
...
```{r function_ibutton_filter_data}
